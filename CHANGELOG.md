...
- **MCP 流式消息乱码**:SSE 响应头 `text/event-stream` 未带 charset,部分
  MCP 客户端不按规范默认 UTF-8 解码导致中文乱码;现在显式
  `charset=utf-8`。
- **面板保存冻结默认值**:保存设置时会把未改动的字段(当时的默认值)一并
  写入覆盖,导致改名后旧默认(如 `.dsh-trio`、`/trio/browser`)一直生效;
  现在只提交与初始值不同的字段。
...
- **描述文案更新**:package.json 与 GitHub 仓库描述从"三个超能力"改为
  如实反映五个模块的新文案(浏览器自动化 + MCP Server + GitHub/GitLab
  自动评审 + 原生嵌入面板)。
