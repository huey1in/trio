// dsh-reef · GitHub — PR 自动评审(LLM 调用、评审去重)
...
async function runReviewLlm(ctx: ReefContext, config: GithubConfig, prompt: string, signal: AbortSignal): Promise<string> {
...
export async function reviewPullRequest(ctx: ReefContext, config: GithubConfig, pr: Record<string, any>) {
